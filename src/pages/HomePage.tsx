...
import type { Coordinates, StationsSearchFilters } from "../types/types";
import ButtonHideList from "../components/ButtonHideList";
import { INITIAL_COORDS, MAX_LIMIT_STATION_TO_FECH, MIN_ZOOM_TO_SEARCH_IN_AREA } from "../constants/constants";
...
import { getDistanceInKm, fetchStationsInBounds, fetchStationsByCoords } from "../services/open-charge-map.service";
...
const HomePage = () => {
  ...
  const { stationsSearchFilters, setStationsSearchFilters } = useStationsSearchFilters();
  ...
  const [isSearchInBoundsDisabled, setIsSearchInBoundsDisabled] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (zoom > MIN_ZOOM_TO_SEARCH_IN_AREA && (!stations.length || distance > 1.5)) {
  ...
  // SEARCH BY COORDS
  const handleSearchByCoords = async (coords: Coordinates, searchInput?: string) => {
      try {
        const newStations = await wrapPromise(fetchStationsByCoords(coords, stationsSearchFilters));
  ...
  const handleSearchInBounds = async () => {
    try {
      const newStations = await wrapPromise(fetchStationsInBounds(bounds, stationsSearchFilters));
      setStations(newStations);
  ...
  return (
    <>
      <div className="relative w-full h-full overflow-hidden">
        ...
        {/* </div> */}
        <SearchByBounds isDisabled={isSearchInBoundsDisabled} handleSearchByBounds={handleSearchInBounds} />
      ...
      <StaggeredDropDown isVisible={isModalOpen}>
        <FilterForm onChangeFilters={handleOnChangeFilters} settedData={stationsSearchFilters} />
      </StaggeredDropDown>
    </>
  )
}
